...
CREATE DEFINER =  `monsignor`@`localhost` TRIGGER `clean_abonnement_after_deleting_dossier` BEFORE DELETE ON  `dossier` FOR EACH ROW DELETE FROM abonnement WHERE user_id = OLD.user_id AND OLD.id = dossier_id;
